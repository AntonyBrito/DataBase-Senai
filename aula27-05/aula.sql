...
INSERT INTO JogadorBrasileiro (cpf, nome, posicao, idEquipe, salario)
VALUES ('064.176.178.09', 'Juca Bala', 'Goleiro', 7, 10900);
...
BEGIN
	IF OLD.salario <> NEW.salario THEN
		INSERT INTO auditoriaJogadorBr (
			idJogadorBr, salarioAntigo, salNovo, usuario, dataAlteracao
        ) VALUE (
        OLD.id, OLD.salario, NEW.salario, CURRENT_USER(), NOW()
        );
	END IF;
END;
//
...
# agora é inserir os dados na tabela esperando o disparo da TRIGGER!
select * from registroCartao;
select * from jogadorBrasileiro;
#1° cartão
INSERT INTO  registroCartao(id_jogador, origem, tipo)
VALUE(1, 'brasileiro', 'amarelo');
#2° cartão
INSERT INTO  registroCartao(id_jogador, origem, tipo)
VALUE(1, 'brasileiro', 'amarelo');
#3° cartão
...
select id, nome, cartoes from jogadorBrasileiro where id = 1;
