...
-- 29 Liste o funcionário que tenha trabalhado a maior quantidade de horas em um projeto
select f.nome, fp.horas from funcionario f, funcionarioProjeto fp 
where f.numero = fp.numeroFunc and
horas = (select max(horas) from funcionarioProjeto);

-- 30 Liste todos os funcionário que trabalham em projetos
select nome from funcionario where numero
in (select numeroFunc from funcionarioProjeto);

-- 31 Liste todos os funcionários que não trabalham em projetos
select nome from funcionario where numero
not in (select numeroFunc from funcionarioProjeto);

-- 32 Liste todos os nomes dos funcionário que trabalham mais horas
-- do que todas as horas trabalhadas por funcionários no projeto 3
select f.nome, fp.horas from funcionario f, funcionarioProjeto fp
where f.numero = fp.numeroFunc and fp.horas =
(select horas from funcionarioProjeto where numeroProj = 3);
